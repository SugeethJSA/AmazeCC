...
export default function MakeupCompreTab({ loginToVTOP, setActiveSubTab }: { loginToVTOP: any; setActiveSubTab: any }) {
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <GenericApiView endpoint="makeup-exam" title="Makeup Exam Eligibility" creds={creds} refreshKey={refreshKey} />
        <GenericApiView endpoint="makeup-schedule" title="Makeup Schedule" creds={creds} refreshKey={refreshKey} />
        <GenericApiView endpoint="compre-info" title="Comprehensive Exam Info" creds={creds} refreshKey={refreshKey} />
      </div>
    </SubpageLayout>
  );
}
